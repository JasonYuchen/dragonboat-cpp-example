...
static const char
  *kExceptionPrefixArgument = "[Zupply Exception->Logic->Argument] ";
...
static const char
  *kExceptionPrefixMemory = "[Zupply Exception->Runtime->Memory] ";
static const char
  *kExceptionPrefixStrictWarn = "[Zupply Exception->StrictWarn] ";
}
...
class LogicException : public Exception {
 public:
  explicit LogicException(const char *message) : Exception(
    message,
    consts::kExceptionPrefixLogic)
  {};
  explicit LogicException(const std::string &message) : Exception(
    message,
    consts::kExceptionPrefixLogic)
  {};
};
...
class ArgException : public Exception {
 public:
  explicit ArgException(const char *message) : Exception(
    message,
    consts::kExceptionPrefixArgument)
  {};
  explicit ArgException(const std::string &message) : Exception(
    message,
    consts::kExceptionPrefixArgument)
  {};
};
...
class RuntimeException : public Exception {
 public:
  explicit RuntimeException(const char *message) : Exception(
    message,
    consts::kExceptionPrefixRuntime)
  {};
  explicit RuntimeException(const std::string &message) : Exception(
    message,
    consts::kExceptionPrefixRuntime)
  {};
};
...
class CastException : public Exception {
 public:
  explicit CastException(const char *message) : Exception(
    message,
    consts::kExceptionPrefixCast)
  {};
  explicit CastException(const std::string &message) : Exception(
    message,
    consts::kExceptionPrefixCast)
  {};
};
...
class IOException : public Exception {
 public:
  explicit IOException(const char *message) : Exception(
    message,
    consts::kExceptionPrefixIO)
  {};
  explicit IOException(const std::string &message) : Exception(
    message,
    consts::kExceptionPrefixIO)
  {};
};
...
class MemException : public Exception {
 public:
  explicit MemException(const char *message) : Exception(
    message,
    consts::kExceptionPrefixMemory)
  {};
  explicit MemException(const std::string &message) : Exception(
    message,
    consts::kExceptionPrefixMemory)
  {};
};
...
class WarnException : public Exception {
 public:
  explicit WarnException(const char *message) : Exception(
    message,
    consts::kExceptionPrefixStrictWarn)
  {};
  explicit WarnException(const std::string &message) : Exception(
    message,
    consts::kExceptionPrefixStrictWarn)
  {};
};
...
class Rect_ {
 public:
  ...
  _Tp x, y, width,
    height; //!< the top-left corner, as well as width and height of the rectangle
};
...
/*!
		 ...
		 */
template<class T>
inline const T clip(const T &value, const T &low, const T &high)
{
...
class Callback {
 public:
  Callback(std::function<void()> f) : f_(f)
  { f_(); }
 private:
...
class UnorderedMap : public UnMovable {
 public:
...
template<typename T>
class NonTrivialContainer : public UnMovable {
 public:
  NonTrivialContainer()
  {}
...
/*!
		 ...
		 */
std::string path_append_basename(
  std::string origPath,
...
class FileEditor : private UnCopyable {
 public:
  ...
  /*!
			 ...
			 */
  FileEditor(
    std::string filename, bool truncateOrNot = false,
  ...
  /*!
			 ...
			 */
  bool open(
    const char *filename, bool truncateOrNot = false,
    int retryTimes = consts::kDefaultFileOpenRetryTimes,
    int retryInterval = consts::kDefaultFileOpenRetryInterval);
 ...
 private:
  ...
  void check_valid()
  { if (!this->is_valid()) throw RuntimeException("Invalid File Editor!"); }
...
class FileReader : private UnCopyable {
 public:
  ...
  /*!
			 ...
			 */
  FileReader(
    std::string filename, int retryTimes = consts::kDefaultFileOpenRetryTimes,
    int retryInterval = consts::kDefaultFileOpenRetryInterval);
 ...
 private:
  ...
  void check_valid()
  { if (!this->is_valid()) throw RuntimeException("Invalid File Reader!"); }
  std::string filename_;
...
class CfgParser {
 public:
  ...
  /*!
			 ...
			 * \param s
			 */
  CfgParser(std::istream &s) : pstream_(&s), ln_(0)
  { parse(root_); }
...
class ArgOption {
 ...
 private:
  ...
  std::vector<std::function<void()>>
    callback_;    //!< call these when option found
  std::vector<std::function<void()>>
    othercall_;    //!< call these when option not found
};
...
class ArgParser {
 ...
 private:
  ...
  std::unordered_map<std::string, std::size_t>
    longKeys_;    //!< long keys --long --version
  std::vector<Value>
    args_;    //!< anything not belong to option will be stored here
  std::vector<std::string> errors_;    //!< store parsing errors
  std::vector<std::string>
    info_;    //!< program name[0] from argv[0], other infos from user
};
...
template<typename T>
inline std::string Value::store(T t)
{
...
template<typename T>
inline std::string Value::store(std::vector<T> t)
{
  std::ostringstream oss;
  for (auto e : t) {
    oss << e << " ";
...
template<typename T>
inline std::vector<T> Value::load(std::vector<T> &t)
{
  std::istringstream iss(str_);
  t.clear();
  T val;
...
template<typename T>
inline ArgOption &ArgParser::add_opt_value(
  ...
  T &dst, T defaultValue, std::string help, std::string type, int min, int max)
{
  ...
  auto &opt = add_opt(shortKey, longKey).call(
    [&, pos]
    { options_[pos].get_value().load(dst); });
...
{
static const char
  *kLevelNames[]{"TRACE", "DEBUG", "INFO", "WARN", "ERROR", "FATAL", "OFF"};
static const char *kShortLevelNames[]{"T", "D", "I", "W", "E", "F", "O"};
static const char *kZupplyInternalLoggerName = "zupply";
static const int
  kAsyncQueueSize = 4096;    //!< asynchrous queue size, must be power of 2
...
static const char
  *kDefaultLoggerFormat = "[%datetime][T%thread][%logger][%level] %msg";
...
class Logger : UnMovable {
 ...
 private:
  ...
  template<typename... Args>
  detail::LineLogger log_if_enabled(
    LogLevels lvl,
    const char *fmt,
    const Args &... args);
...
class LineLogger : private UnCopyable {
 public:
  LineLogger(Logger *callbacker, LogLevels lvl, bool enable) : callbackLogger_(
    callbacker), enabled_(enable)
  {
    msg_.level_ = lvl;
  }
 ...
 private:
  std::string format_message(const LogMessage &msg)
  {
    ...
    fmt::replace_all_with_escape(
      ret,
      consts::kSinkDatetimeSpecifier,
      dt.to_string(LogConfig::instance().datetime_format().c_str()));
    fmt::replace_all_with_escape(
      ret,
      consts::kSinkLoggerNameSpecifier,
      msg.loggerName_);
    fmt::replace_all_with_escape(
      ret,
      consts::kSinkThreadSpecifier,
      std::to_string(msg.threadId_));
    fmt::replace_all_with_escape(
      ret,
      consts::kSinkLevelSpecifier,
      consts::kLevelNames[msg.level_]);
    fmt::replace_all_with_escape(
      ret,
      consts::kSinkLevelShortSpecifier,
      consts::kShortLevelNames[msg.level_]);
    fmt::replace_all_with_escape(
      ret,
      consts::kSinkMessageSpecifier,
      msg.buffer_);
    // make sure new line
...
class SimpleFileSink : public Sink {
 public:
  SimpleFileSink(const std::string filename, bool truncate) : fileEditor_(
    filename,
    truncate)
  {
  }
...
class StdoutSink : public OStreamSink {
 public:
  ...
  {
    static std::shared_ptr<StdoutSink>
      instance = std::make_shared<StdoutSink>();
    return instance;
  }
...
class StderrSink : public OStreamSink {
 public:
  ...
  {
    static std::shared_ptr<StderrSink>
      instance = std::make_shared<StderrSink>();
    return instance;
  }
...
template<typename... Args>
inline detail::LineLogger Logger::log_if_enabled(
  LogLevels lvl,
  const char *fmt,
  const Args &... args)
{
...
{ return _Tp(v); }
template<typename _Tp>
static inline _Tp saturate_cast(unsigned short v)
{ return _Tp(v); }
template<typename _Tp>
...
template<>
inline unsigned char saturate_cast<unsigned char>(unsigned char v)
...
template<>
inline char saturate_cast<char>(unsigned short v)
{ return (char) (std::min)((unsigned) v, (unsigned) SCHAR_MAX); }
template<>
inline char saturate_cast<char>(int v)
{
  return (char) ((unsigned) (v - SCHAR_MIN) <= (unsigned) UCHAR_MAX ? v : v > 0
                                                                          ? SCHAR_MAX
                                                                          : SCHAR_MIN);
}
template<>
inline char saturate_cast<char>(short v)
{ return saturate_cast<char>((int) v); }
template<>
inline char saturate_cast<char>(unsigned int v)
...
template<>
inline unsigned short saturate_cast<unsigned short>(char v)
...
{
  return (unsigned short) ((unsigned) v <= (unsigned) USHRT_MAX ? v : v > 0
                                                                      ? USHRT_MAX
                                                                      : 0);
}
template<>
...
{
  return (short) ((unsigned) (v - SHRT_MIN) <= (unsigned) USHRT_MAX ? v : v > 0
                                                                          ? SHRT_MAX
                                                                          : SHRT_MIN);
}
template<>
...
void ImageBase<_Tp>::range_check(int row, int col, int channel) const
{
  ...
  if (row >= rows_ || col >= cols_ || channel >= channels_)
    throw RuntimeException("Access out of range!");
}
...
{
  assert(
    rows > 0 && cols > 0 && channels > 0 && "import size should be positive");
  create(rows, cols, channels);
...
{
  assert(r0 >= 0 && c0 >= 0 && r1 >= 0 && c1 >= 0
    && "crop region should be positive!");
  assert(r0 < rows_ && r1 < rows_ && c0 < cols_ && c1 < cols_
